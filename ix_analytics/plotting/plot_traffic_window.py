...
def _ts_to_datetime(ts, tz):
    ...
    return datetime.utcfromtimestamp(ts)+timedelta(hours=tz)
...
def plot(data, srcPort, dstAS, start_time, end_time, timezone, vol_th=5):
    ...
    bts = datetime.strptime(plot_begin_time, _TIME_STR_FORMAT)
    ets = datetime.strptime(plot_end_time, _TIME_STR_FORMAT)

    bts = time.mktime(bts.timetuple()) + timezone*3600
    ets = time.mktime(ets.timetuple()) + timezone*3600
    ...
    sql_query = """
                ...
                GROUP BY time, srcAS
                ORDER BY time;
                """ % (srcport, dstAS, bts, ets)

    attack_info = pdsql.sqldf(sql_query, locals())

    print("attack_info.shape =", attack_info.shape)
    ...
    for srcAS, max_vol in sorted_srcAS_peak_vol[:_TOP_SRCAS_LIMIT]:
        ...
        time_col = [_ts_to_datetime(t, 0) for t in time_col]
        attack_srcAS['time'] = [_truncate_time_to_min(t) for t in time_col] 
        att = attack_srcAS.set_index('time')
        att = att.astype('float')

        min_time = datetime.strptime(plot_begin_time, _TIME_STR_FORMAT)
        max_time = datetime.strptime(plot_end_time, _TIME_STR_FORMAT)
