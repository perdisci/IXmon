...
std::unordered_map<std::string, uint32_t> sampling_rate_map;
...
int process_netflow_v10_options_template(u_int8_t* pkt, size_t len, u_int32_t source_id) {
    ...
    logger << log4cpp::Priority::DEBUG << "Options template id: " << template_id
...
/* NOTE(Roberto): Added for convenience... */
#define COPY32_NTOHL(a) memcpy((u_char*)&a, data, record_length); a=ntohl(a);
/*********/
...
int nf9_rec_to_flow(u_int record_type, u_int record_length, u_int8_t* data, simple_packet& packet) {
    ...
    switch (record_type) {
        ...
        V9_FIELD_ADDR(NF9_IPV4_DST_ADDR, SRC_ADDR4, dst_ip);

        /* NOTE(Roberto): Add AS info, if present */

        // The code below is repetitive, and could definitely be optimized
        case NF9_INPUT_SNMP:
            COPY32_NTOHL(packet.ifinf.in_idx);
            break;

        case NF9_OUTPUT_SNMP:
            COPY32_NTOHL(packet.ifinf.out_idx);
            break;

        case NF9_SRC_AS:
            COPY32_NTOHL(packet.asinf.src_as);

            /* NOTE(Roberto): in some cases, when the Router is not able to identify
             *      the correct AS number, it may set it to UINT16_MAX or UINT32_MAX
             *      (probably because it is meant to be set to -1)
             *      In thse cases, we reset the AS number to zero.
             */
            if(packet.asinf.src_as == UINT16_MAX or packet.asinf.src_as == UINT32_MAX)
                packet.asinf.src_as = 0;

            /* NOTE(Roberto): log info for debugging purposes only...
            in_addr sip;
            sip.s_addr = packet.src_ip;
            logger << log4cpp::Priority::INFO << "---FNM Print: " << print_simple_packet(packet);
            logger << log4cpp::Priority::INFO << "+++NFv10 ==> SRC_IP = " << inet_ntoa(sip) 
                   << " | SRC_AS = " << packet.asinf.src_as << " | IN_IF = " << packet.ifinf.in_idx;
            */
            break;

        case NF9_DST_AS:
            COPY32_NTOHL(packet.asinf.dst_as);
             ...
             *      In thse cases, we reset the AS number to zero.
             */
            if(packet.asinf.dst_as == UINT16_MAX or packet.asinf.dst_as == UINT32_MAX)
                packet.asinf.dst_as = 0;
    ...
    /* NOTE(Roberto): Only needed for Debugging purposes info */
    switch(record_type) {
        case NF9_IPV4_SRC_ADDR:
            in_addr sip;
            sip.s_addr = packet.src_ip;
            logger << log4cpp::Priority::DEBUG << "+++NFv9 ==> SRC_IP = " << inet_ntoa(sip);
            //logger << log4cpp::Priority::INFO << "---Packet: " << print_simple_packet(packet);
            //logger << log4cpp::Priority::INFO << "+++NFv9 ==> SRC_IP = " << inet_ntoa(sip);
            break;
	    
        case NF9_IPV4_DST_ADDR:
            in_addr dip;
            dip.s_addr = packet.dst_ip;
            logger << log4cpp::Priority::DEBUG << "+++NFv9 ==> DST_IP = " << inet_ntoa(dip);
	        //logger << log4cpp::Priority::INFO << "---Packet: " << print_simple_packet(packet);
	        //logger << log4cpp::Priority::INFO << "+++NFv9 ==> DST_IP = " << inet_ntoa(dip);
            break;
	    
        case NF9_L4_DST_PORT:
	        logger << log4cpp::Priority::DEBUG << "+++NFv9 ==> DST_PORT = " << packet.destination_port;
	        //logger << log4cpp::Priority::INFO << "---Packet: " << print_simple_packet(packet);
            //logger << log4cpp::Priority::INFO << "+++NFv9 ==> DST_PORT = " << packet.destination_port;
	    break;
    }
...
int nf10_rec_to_flow(u_int record_type, u_int record_length, u_int8_t* data, simple_packet& packet) {
    ...
    switch (record_type) {
        ...
        case NF10_DST_AS:
             ...
             *      the correct AS number, it may set it to UINT16_MAX or UINT32_MAX
             *      (probably because it is meant to be set to -1)
             *      In these cases, we reset the AS number to zero.
             */
            if(packet.asinf.dst_as == UINT16_MAX or packet.asinf.dst_as == UINT32_MAX)
                packet.asinf.dst_as = 0;
    ...
    switch(record_type) {
        case NF10_IPV4_SRC_ADDR:
            in_addr sip;
            sip.s_addr = packet.src_ip;
            logger << log4cpp::Priority::DEBUG << "+++NFv10 ==> SRC_IP = " << inet_ntoa(sip);
            break;
        case NF10_IPV4_DST_ADDR:
            in_addr dip;
            dip.s_addr = packet.dst_ip;
            logger << log4cpp::Priority::DEBUG << "+++NFv10 ==> DST_IP = " << inet_ntoa(dip);
            break;
    }
...
void nf10_flowset_to_store(u_int8_t* pkt, size_t len, 
                           struct NF10_HEADER* nf10_hdr, struct peer_nf9_template* field_template,
                           std::string &client_addres_in_string_format, u_int32_t source_id) {
    u_int offset = 0;
    ...
    /* NOTE(Roberto): added information on source of NetFlow information */
    packet.nf_source_client_addr = client_addres_in_string_format;
    packet.nf_source_id = source_id;
    logger << log4cpp::Priority::DEBUG << "--->(Roberto) :: " 
           << " nf_source_client_addr = " << packet.nf_source_client_addr
           << " nf_source_id = " << packet.nf_source_id;



    /* TODO(Roberto): the code should not be added here, but rather where both the NFsender and SourceID can be accessed;
        In alternative, we could pass the NFsender as a parameter to this function

        here we should record the SOURCE ID and SRC_IP of the router
        that exported the logs. This, paired with INPUT_SNMP, should give us a change
        to build a unique ID for the input interface from which the flow was collected
        
        See the following link:
        https://www.cisco.com/en/US/technologies/tk648/tk362/technologies_white_paper09186a00800a3db9.html
    */ 

    auto sr = sampling_rate_map.find(packet.nf_source_client_addr);
    if(sr != sampling_rate_map.end()) {
        // std::cout << "===> v10 -- CUSTOM SAMPLING RATE: " << sr->first << " = " << sr->second << std::endl;
        packet.sample_ratio = sr->second;
    }
    else {
    ...
    /* NOTE(Roberto): I'm not sure why the original code makes the conversion
        between network and host byte order here, instead of directly inside nf10_rec_to_flow()
        We convert the AS and SNMP interface information there, so no need to do it again here...
    */
...
void nf9_flowset_to_store(u_int8_t* pkt, size_t len, 
                          struct NF9_HEADER* nf9_hdr, netflow9_template_records_map& template_records, 
                          std::string &client_addres_in_string_format, u_int32_t source_id) {
    ...
    auto sr = sampling_rate_map.find(packet.nf_source_client_addr);
    if(sr != sampling_rate_map.end()) {
        // std::cout << "===> v9 -- CUSTOM SAMPLING RATE: " << sr->first << " = " << sr->second << std::endl;
        packet.sample_ratio = sr->second;
    }
    else {
        packet.sample_ratio = sampling_rate;
    ...
    // NOTE(Roberto): testing...
    if(packet.source_port == 53)
        logger << log4cpp::Priority::ERROR << "Found source port = 53!!!";
...
int process_netflow_v10_data(u_int8_t* pkt,
    ...
    if (len < sizeof(*dath)) {
        logger << log4cpp::Priority::DEBUG << "Short netflow v10 data flowset header";
        return 1;
    }
    ...
    if (flowset_template == NULL) {
        logger << log4cpp::Priority::DEBUG << "We don't have a template for flowset_id: " << flowset_id
               << " but it's not an error if this message disappears in 4-10 seconds. We need some "
                  "time to learn it!";
    ...
    for (u_int i = 0; i < num_flowsets; i++) {
        ...
        nf10_flowset_to_store(pkt + offset, flowset_template->total_len, nf10_hdr, flowset_template, client_addres_in_string_format, source_id);
...
int process_netflow_v9_data(u_int8_t* pkt, size_t len, 
                            struct NF9_HEADER* nf9_hdr, u_int32_t source_id, 
                            std::string client_addres_in_string_format) {
    ...
    if (flowset_template == NULL) {
        logger << log4cpp::Priority::DEBUG << "We don't have a template for flowset_id: " << flowset_id
               << " but it's not an error if this message disappears in 6-10 seconds. We need some "
                  "time to learn it!";
        return 0;
    }
    ...
    for (u_int i = 0; i < num_flowsets; i++) {
        // process whole flowset
        nf9_flowset_to_store(pkt + offset, flowset_template->total_len, nf9_hdr, flowset_template->records, client_addres_in_string_format, source_id);
...
void process_netflow_packet_v10(u_int8_t* packet, u_int len, std::string &client_addres_in_string_format) {
    ...
    for (i = 0;; i++) {
        ...
        case NF10_TEMPLATE_FLOWSET_ID:
            // std::cout << "Calling v10 template processing!" << std::endl;
        ...
        default:
            ...
            // std::cout << "Calling v10 data processing!" << std::endl;
...
void process_netflow_packet_v9(u_int8_t* packet, u_int len, std::string client_addres_in_string_format) {
    ...
    for (u_int32_t i = 0;; i++) {
        ...
        case NF9_TEMPLATE_FLOWSET_ID:
            // std::cout << "This is a v9 template! " << flowset_len << std::endl;
    ...
    // std::cout << "Finished process_netflow_packet_v9" << std::endl;
}
...
void process_netflow_packet_v5(u_int8_t* packet, u_int len, std::string client_addres_in_string_format) {
    ...
    for (u_int i = 0; i < nflows; i++) {
        ...
        /* TODO(Roberto): Add information about Src/Dst-ASes and SNMP interfaces */
        /* See v9 and v10 code, for guidance */
...
void process_netflow_packet(u_int8_t* packet, u_int len, std::string client_addres_in_string_format) {
    ...
    case 5:
        // std::cout << "This is a v5 packet!" << std::endl;
        // TODO(Roberto):
        // NFv5 is currently not supported by IXmon
        // We need to modify process_netflow_packet_v5() in order to 
        // extract the traffic info needed by IXmon.
        // process_netflow_packet_v5(packet, len, client_addres_in_string_format);
        break;
    case 9:
        // std::cout << "This is a v9 packet!" << std::endl;
        process_netflow_packet_v9(packet, len, client_addres_in_string_format);
        break;
    case 10:
        // std::cout << "This is a v10 packet!" << std::endl;
        process_netflow_packet_v10(packet, len, client_addres_in_string_format);
        break;
    default:
        // std::cout << "The version of this packet is not supported!" << std::endl;
...
std::unordered_map<std::string,uint32_t> read_sampling_rate_map(std::string& map_fn) {
    std::unordered_map<std::string,uint32_t> srmap;

    // TODO(Roberto): open file map_fn and read sampling rate configuration
    std::ifstream f(map_fn, std::ios::binary);
    if (!f.is_open()) {
        std::cout << "failed to open " << map_fn << '\n';
        logger << log4cpp::Priority::ERROR << "FAILED TO OPEN sampling_rate_map FILE: " << map_fn;
        return srmap;
    } 
    ...
    while(std::getline(f,line)) {
        ...
        // IP_Addr samplin_rate
        // Example:
        // 10.0.0.1 1000

        if(line[0]=='#') // skip comment
            continue; 
        char* dup = strdup(line.c_str()); // remember to free dup!
        char* token = std::strtok(dup," ");
        if(token == NULL) // must be invalid line; skip it!
            continue;
        std::string nf_client_ip(token);
        token = std::strtok(NULL," ");
        if(token == NULL) // missing sampling rate info; invlid line; skip it!
            continue;
        std::string nf_sampling_rate_str(token);
        uint32_t nf_sampling_rate = std::stoul(nf_sampling_rate_str);
        free(dup); // free dup!

        logger << log4cpp::Priority::INFO << "Custom sampling rate found: " << nf_client_ip << " = " << nf_sampling_rate;

        srmap.insert({nf_client_ip,nf_sampling_rate});
    }

    return std::move(srmap);
}
...
void start_netflow_collection(process_packet_pointer func_ptr) {
    ...
    if (configuration_map.count("default_netflow_sampling_ratio") != 0) {
        sampling_rate = convert_string_to_integer(configuration_map["default_netflow_sampling_ratio"]);

        logger << log4cpp::Priority::INFO << "Using custom sampling ratio for netflow: " << sampling_rate;
    }

    if (configuration_map.count("netflow_sampling_ratio_map_file") != 0) {
        std::string map_fn = configuration_map["netflow_sampling_ratio_map_file"];
        sampling_rate_map = read_sampling_rate_map(configuration_map["netflow_sampling_ratio_map_file"]);

        logger << log4cpp::Priority::INFO << "Reading sampling rate map from: " << map_fn;
    }
...
void start_netflow_collector(std::string netflow_host, unsigned int netflow_port) {
    ...
    while (true) {
        ...
        if (received_bytes > 0) {
            ...
            /* NOTE(Roberto): the following optimizes the above assignment line.
                Also, we have optimized most NFv10 function calls by passing 
                client_addres_in_string_format by reference, instead of value,
                to avoid copying the same string over and over again.
            */
            std::string client_addres_in_string_format(host);

            // std::cout << "Received packets from: " << client_addres_in_string_format << std::endl;
