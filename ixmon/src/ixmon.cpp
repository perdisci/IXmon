...
/* NOTE(Roberto): */
#include <boost/asio.hpp> // needed for exporting info via udp
#include "libpatricia/patricia_ip_asn.h"

#define IXMON_LOG_PRIORITY log4cpp::Priority::INFO
/*********/
...
/* NOTE(Roberto): needed to dump AS-to-PortID export */
boost::thread_group ix_traffic_thread_group;
time_t ix_traffic_matrix_dump_timeout = 60; // default: 1 minute
bool ixmon_udp_export = false;
std::string ixmon_expfile_template = "/tmp/ixmon-export.XXXXXX";
/*********/

// Total number of hosts in our networks
// We need this as global variable because it's very important value 
// for configuring data structures
...
/* NOTE(Roberto): IP-to-AS patricia tree */
patricia_tree_t* ip_asn_tree_ptr;
boost::mutex patricia_ip_asn_tree_mutex;
time_t update_patricia_ip_asn_tree_timeout = 24*60*60; // daily update
/*********/

/* NOTE(Roberto): AS-to-PortID matrix */
// we use two src_asif_matrix maps to allow for swapping between the two
// when we perform a dump+reset in a separate thread without blocking
// the collection of new stats in the following time window
as_nfif_matrix_counters src_asif_matrix[2];
// the index to the active src_asif_matrix (can only be 0 or 1)
bool src_asif_matrix_active_idx = 0; 
as_nfif_matrix_counters *src_asif_matrix_ptr = 
                                &src_asif_matrix[src_asif_matrix_active_idx];
boost::mutex src_asif_matrix_mutex;
/*********/


/* NOTE(Roberto): srcAS-to-dstAS matrix */
// we use two src_asif_matrix maps to allow for swapping between the two
// when we perform a dump+reset in a separate thread without blocking
// the collection of new stats in the following time window
as_as_matrix_counters as_as_matrix[2];
// the index to the active src_asif_matrix (can only be 0 or 1)
bool as_as_matrix_active_idx = 0;
as_as_matrix_counters *as_as_matrix_ptr =
                            &as_as_matrix[as_as_matrix_active_idx];
boost::mutex as_as_matrix_mutex;
/*********/

// NOTE(Roberto): NetFlow clients whose traffic will be accepted
std::set<std::string> nf_clients_set;


/* NOTE(Roberto): port-specific stats */
// TODO(Roberto): make these lists configurable
// Interesting UDP ports:
// https://blog.cloudflare.com/reflections-on-reflections/
// NTP = 123, SSDP = 1900, DNS=53, SRCDS=27015, Chargen=19,
// CallOfDuty = 20800, SNMP=161, CLDAP=389, Sunrpc=111, NETBIOS=137
// SRCDS=27005, RIP=520
// https://blog.cloudflare.com/\\
// memcrashed-major-amplification-attacks-from-port-11211/
// Memcached=11211
// Additional ports based on 
// https://christian-rossow.de/publications/amplification-ndss2014.pdf
// QOTD=17, Quake3=27960, Steam=29015
// https://www.akamai.com/fr/fr/multimedia/documents/state-of-the-internet/\
// ms-sql-server-reflection-ddos-mc-sqlr-threat-advisory.pdf
// MC-SQLR=1434
// https://www.akamai.com/us/en/about/our-thinking/threat-advisories/\
// akamai-mdns-reflection-ddos-threat-advisory.jsp
// mDNS=5353
std::set<uint16_t> udp_interesting_ports =
      {123, 1900, 53, 27015, 19, 20800, 
       161, 389, 111, 137, 27005, 520, 11211,
       17, 27960, 29015, 1434, 5353};

std::set<uint16_t> tcp_interesting_ports = {80, 443, 25, 22};
/*********/


// Set of ASes that should be tracked at the subnet granularity
std::set<std::uint32_t> subnet_tracking_ASes;
std::string subnet_tracking_ASes_list_path;
...
//Maria-edits
bool load_subnet_tracking_ASes();
...
bool load_configuration_file() {
    ...
    /* NOTE(Roberto): allow for configuring timeouts */
    if(configuration_map.count("ix_traffic_matrix_dump_timeout") != 0) {
        ix_traffic_matrix_dump_timeout = convert_string_to_integer(configuration_map["ix_traffic_matrix_dump_timeout"]);
    }

    if(configuration_map.count("update_patricia_ip_asn_tree_timeout") != 0) {
        update_patricia_ip_asn_tree_timeout = convert_string_to_integer(configuration_map["update_patricia_ip_asn_tree_timeout"]);
    }
    /************/
    ...
    //Maria-edtis, check if the user has added specific addiotinal ASes. If yes, then we can track 24s-to-24s of those additional ASes, if they appear as srcASes or destinations. This is in addition. The default is to track AS-to-AS.
    // * * * * *
    if (configuration_map.count("subnet_tracking_ASes") != 0) {
      if (configuration_map["subnet_tracking_ASes"] == "on" or configuration_map["subnet_tracking_ASes"] == "yes") {
        subnet_tracking_ASes_list_path = configuration_map["subnet_tracking_ASes_path"];
      }
    }

    if (configuration_map.count("nf_clients") != 0) {
        std::string nf_clients_str = configuration_map["nf_clients"];
        std::vector<std::string> v;
        boost::split(v, nf_clients_str, boost::is_any_of(","), boost::token_compress_on);
        nf_clients_set.insert(v.begin(), v.end());
        for(auto v : nf_clients_set)
            printf("Allow nf_client=%s\n", v.c_str());
    }
    // * * * * *
...
//Maria-edits: load our list of ASes.
// * * *  * * *  * * *  * * *  * * *  * * *  * * *  * * *  * * *  * * *  * * *
bool load_subnet_tracking_ASes() {

  if (file_exists(subnet_tracking_ASes_list_path)) {

    std::ifstream as_file;
    as_file.open(subnet_tracking_ASes_list_path.c_str(), std::ifstream::in);

    std::string line;
    if (as_file.is_open()) {
      while (getline(as_file, line)) {
        boost::algorithm::trim(line);
        uint32_t as_number = stoi(line);
	      subnet_tracking_ASes.insert(as_number);
      }
    } else {
      logger << log4cpp::Priority::ERROR << "Can't open file: "
             << subnet_tracking_ASes_list_path.c_str();
    }
  } else {
      logger << log4cpp::Priority::ERROR << "Can't open file: "
             << subnet_tracking_ASes_list_path.c_str();
  ...
  logger << log4cpp::Priority::INFO << "Loaded " << subnet_tracking_ASes.size()
         << " ASes from: " << subnet_tracking_ASes_list_path.c_str();
...
void update_ix_traffic_counters(ix_traffic_counters &cnt, 
                                uint32_t sampled_number_of_packets,
                                uint32_t sampled_number_of_bytes, 
                                uint8_t flags, 
                                unsigned int protocol,
                                uint16_t src_port,
                                uint16_t dst_port) {

        cnt.num_packets += sampled_number_of_packets;
        cnt.num_bytes += sampled_number_of_bytes;

        if (protocol == IPPROTO_TCP) {
            uint32_t sampled_number_of_syn_packets = 0;
            if (extract_bit_value(flags, TCP_SYN_FLAG_SHIFT))
                sampled_number_of_syn_packets += 1;

            cnt.tcp_packets += sampled_number_of_packets;
            cnt.tcp_syn_packets += sampled_number_of_syn_packets;
            cnt.tcp_bytes += sampled_number_of_bytes;

            if(tcp_interesting_ports.find(src_port) !=
                                    tcp_interesting_ports.end()) {
                if(DEBUG) logger << log4cpp::Priority::DEBUG 
                                 << "Found interesting TCP src_port= " 
                                 << src_port;

                auto p = cnt.tcp_sport_stats.find(src_port);
                if(p != cnt.tcp_sport_stats.end()) {
                    std::get<0>(p->second) += sampled_number_of_packets;
                    std::get<1>(p->second) += sampled_number_of_syn_packets;
                    std::get<2>(p->second) += sampled_number_of_bytes;
                }    
                else {
                    cnt.tcp_sport_stats.emplace(src_port, 
                        std::make_tuple(sampled_number_of_packets,
                                        sampled_number_of_syn_packets,
                                        sampled_number_of_bytes));
                }
            }
            if(tcp_interesting_ports.find(dst_port) !=
                                    tcp_interesting_ports.end()) {
                if(DEBUG) logger << log4cpp::Priority::DEBUG 
                                 << "Found interesting TCP dst_port= " 
                                 << dst_port;

                auto p = cnt.tcp_dport_stats.find(dst_port);
                if(p != cnt.tcp_dport_stats.end()) {
                    std::get<0>(p->second) += sampled_number_of_packets;
                    std::get<1>(p->second) += sampled_number_of_syn_packets;
                    std::get<2>(p->second) += sampled_number_of_bytes;
                }    
                else {
                    cnt.tcp_dport_stats.emplace(dst_port, 
                        std::make_tuple(sampled_number_of_packets,
                                        sampled_number_of_syn_packets,
                                        sampled_number_of_bytes));
                }
            ...
            cnt.udp_packets += sampled_number_of_packets;
            cnt.udp_bytes += sampled_number_of_bytes;

            if(udp_interesting_ports.find(src_port) !=
                                    udp_interesting_ports.end()) {
                if(DEBUG) logger << log4cpp::Priority::DEBUG 
                                 << "Found interesting UDP src_port= " 
                                 << src_port;

                auto p = cnt.udp_sport_stats.find(src_port);
                if(p != cnt.udp_sport_stats.end()) {
                    std::get<0>(p->second) += sampled_number_of_packets;
                    std::get<1>(p->second) += sampled_number_of_bytes;
                }    
                else {
                    cnt.udp_sport_stats.emplace(src_port, 
                        std::make_tuple(sampled_number_of_packets,
                                        sampled_number_of_bytes));
                }
            }
            if(udp_interesting_ports.find(dst_port) !=
                                    udp_interesting_ports.end()) {
                if(DEBUG) logger << log4cpp::Priority::DEBUG 
                                 << "Found interesting UDP dst_port= " 
                                 << dst_port;

                auto p = cnt.udp_dport_stats.find(dst_port);
                if(p != cnt.udp_dport_stats.end()) {
                    std::get<0>(p->second) += sampled_number_of_packets;
                    std::get<1>(p->second) += sampled_number_of_bytes;
                }
                else {
                    cnt.udp_dport_stats.emplace(dst_port, 
                        std::make_tuple(sampled_number_of_packets,
                                        sampled_number_of_bytes));
                }
            }
            ...
            cnt.icmp_packets += sampled_number_of_packets;
            cnt.icmp_bytes += sampled_number_of_bytes;
        }

        else  {
            if(DEBUG) logger << log4cpp::Priority::DEBUG 
                             << ">>> OTHER IP PROTOCOL = " << protocol;
        }
        if(DEBUG) logger << log4cpp::Priority::DEBUG 
                         << "Counters = " << cnt.str();
}
...
uint32_t patricia_ip4_asn_get(uint32_t ip) {

    prefix_t test_prefix;
    test_prefix.family = AF_INET;
    test_prefix.bitlen = 32;
    test_prefix.add.sin.s_addr = ip;

    patricia_ip_asn_tree_mutex.lock();
    patricia_node_t* p_node = patricia_search_best2(ip_asn_tree_ptr, &test_prefix, 1);
    patricia_ip_asn_tree_mutex.unlock();

    uint32_t asn = 0;
    if(p_node) asn = p_node->uint32_data;

    if(DEBUG) logger << log4cpp::Priority::DEBUG << "===> IP-to-ASN: " << ip << " => " << asn;
...
void process_packet(simple_packet& current_packet) {
    ...
    // NOTE(Roberto): process only packets from allowed nf_clients (see ixmon.conf)
    if(nf_clients_set.find(current_packet.nf_source_client_addr) == nf_clients_set.end()) 
        return;

    // NOTE(Roberto): directory for ixmon stats export files
    if (configuration_map.count("ixmon_expfile_template") != 0) {
        ixmon_expfile_template = configuration_map["ixmon_expfile_template"];
    }

    /* NOTE(Roberto): use patricia tree for efficient ip-to-asn mapping */

    if(DEBUG) logger << log4cpp::Priority::DEBUG
                     << "Using Patricia Tree for packet: " 
                     << print_simple_packet(current_packet);

    // printf("srcAS=%d, srcIP=%x\n",current_packet.asinf.src_as,ntohl(current_packet.src_ip));
    if(current_packet.asinf.src_as == 0) {
        current_packet.asinf.src_as = patricia_ip4_asn_get(current_packet.src_ip);
        // printf("Patricia: srcAS=%d, srcIP=%x\n",current_packet.asinf.src_as,ntohl(current_packet.src_ip));
    }
    // printf("dstAS=%d, dstIP=%x\n",current_packet.asinf.dst_as,ntohl(current_packet.dst_ip));
    if(current_packet.asinf.dst_as == 0) {
        current_packet.asinf.dst_as = patricia_ip4_asn_get(current_packet.dst_ip);
        // printf("Patricia: dstAS=%d, dstIP=%x\n",current_packet.asinf.dst_as,ntohl(current_packet.dst_ip));
    }
    /***************/
    ...
    */
    ...
    /* NOTE(Roberto): This is new code for computing
     * AS-to-PortID matrix counters -- only SRC informaion for now */
    /* TODO(Roberto): This code is limited to NetFlow, and needs
     * to be generalized to support sFlow and raw packets */

    // First build as_nfif_pair from current_packet info
    as_nfif_pair src_asif;
    src_asif.srcAS =  current_packet.asinf.src_as;
    src_asif.dstAS =  current_packet.asinf.dst_as;
    src_asif.if_idx.src_id = current_packet.nf_source_id;
    src_asif.if_idx.if_id = current_packet.ifinf.in_idx;
    src_asif.if_idx.nf_client_ip_addr = current_packet.nf_source_client_addr;

    auto srcas_find = subnet_tracking_ASes.find(src_asif.srcAS);
    if(srcas_find != subnet_tracking_ASes.end()) {
        src_asif.srcIP24 = ntohl(current_packet.src_ip) & SUBNET_MASK_24;
        // printf("if -- srcAS=%d, srcIP=%x, srcIP24 = %x, mask=%x\n",
        //         src_asif.srcAS, ntohl(current_packet.src_ip), 
        //         src_asif.srcIP24, SUBNET_MASK_24);
    }
    auto dstas_find = subnet_tracking_ASes.find(src_asif.dstAS);
    if(dstas_find != subnet_tracking_ASes.end()) {
        src_asif.dstIP24 = ntohl(current_packet.dst_ip) & SUBNET_MASK_24;
        // printf("if -- dstAS=%d, dstIP=%x, dstIP24 = %x, mask=%x\n",
        //         src_asif.dstAS, ntohl(current_packet.dst_ip), 
        //         src_asif.dstIP24, SUBNET_MASK_24);
    }

    // Check if as_nfif_pair is already in the matrix
    // NOTE(Roberto): we need to be extra careful about thread safety here
    // because there may be multiple NetFlow collectors running
    // (e.g., on different UDP ports)

    src_asif_matrix_mutex.lock();
    auto asif_itr = src_asif_matrix_ptr->find(src_asif);
    if(asif_itr != src_asif_matrix_ptr->end()) {
        if(DEBUG) logger << log4cpp::Priority::DEBUG
                         << "Found AS-to-PortID matrix entry!";
        if(DEBUG) logger << log4cpp::Priority::DEBUG
                         << "AS-to-PortID entry = " << src_asif.str();
        // itr->first is an as_nfif_pair data structure
        // itr->second is an ix_traffic_counters data structure

        update_ix_traffic_counters(asif_itr->second,
                                   sampled_number_of_packets,
                                   sampled_number_of_bytes,
                                   current_packet.flags,
                                   current_packet.protocol,
                                   current_packet.source_port,
                                   current_packet.destination_port);
    }
    else {
        if(DEBUG) logger << log4cpp::Priority::DEBUG
                         << "Not found AS-to-PortID matrix entry!";
        if(DEBUG) logger << log4cpp::Priority::DEBUG
                         << "AS-to-PortID entry = " << src_asif.str();
        ix_traffic_counters cnt;
        update_ix_traffic_counters(cnt, sampled_number_of_packets,
                                   sampled_number_of_bytes,
                                   current_packet.flags,
                                   current_packet.protocol,
                                   current_packet.source_port,
        ...
        src_asif_matrix_ptr->insert({src_asif, cnt});
        if(DEBUG) logger << log4cpp::Priority::DEBUG
                         << "Created AS-to-PortID matrix entryi!";
    }
    src_asif_matrix_mutex.unlock();

    /****************/

    /* NOTE(Roberto): This is new code
     * for computing srcAS-to-dstAS matrix counters */

    // First build as_as_pair from current_packet info

    as_as_pair sas_das;
    sas_das.srcAS = current_packet.asinf.src_as;
    sas_das.dstAS = current_packet.asinf.dst_as;
    sas_das.nf_client_addr = current_packet.nf_source_client_addr;

    auto srcAS_search = subnet_tracking_ASes.find(sas_das.srcAS);
    if(srcAS_search != subnet_tracking_ASes.end()) {
        sas_das.srcIP24 = ntohl(current_packet.src_ip) & SUBNET_MASK_24;
        // printf("asas -- srcAS=%d, srcIP=%x, srcIP24 = %x, mask=%x\n",
        //         src_asif.srcAS, ntohl(current_packet.src_ip),
        //         src_asif.srcIP24, SUBNET_MASK_24);
    }

    auto dstAS_search = subnet_tracking_ASes.find(sas_das.dstAS);
    if(dstAS_search != subnet_tracking_ASes.end()) {
        sas_das.dstIP24 = ntohl(current_packet.dst_ip) & SUBNET_MASK_24;
        // printf("asas -- dstAS=%d, dstIP=%x, dstIP24 = %x, mask=%x\n",
        //         src_asif.dstAS, ntohl(current_packet.dst_ip),
        //         src_asif.dstIP24, SUBNET_MASK_24);
    }

    logger << log4cpp::Priority::DEBUG << "--- Checking: sas_das entry: " << sas_das.str();

    // Check if as_as_pair is already in the matrix
    as_as_matrix_mutex.lock();
    auto asas_itr = as_as_matrix_ptr->find(sas_das);

    if(asas_itr != as_as_matrix_ptr->end()) {
        if(DEBUG) logger << log4cpp::Priority::DEBUG
                         << "Found srcAS-to-dstAS matrix entry!";
        if(DEBUG) logger << log4cpp::Priority::DEBUG
                         << "AS-to-AS entry = " << sas_das.str();

        // itr->first is an as_as_pair data structure
        // itr->second is an ix_traffic_counters data structure

        update_ix_traffic_counters(asas_itr->second,
                                   sampled_number_of_packets,
                                   sampled_number_of_bytes,
                                   current_packet.flags,
                                   current_packet.protocol,
        ...
        if(DEBUG) logger << log4cpp::Priority::DEBUG
                         << "Not found srcAS-to-dstAS matrix entry!";
        if(DEBUG) logger << log4cpp::Priority::DEBUG
                         << "AS-to-AS entry = " << sas_das.str();
        ix_traffic_counters cnt;
        update_ix_traffic_counters(cnt, sampled_number_of_packets,
        ...
        as_as_matrix_ptr->insert({sas_das, cnt});
        if(DEBUG) logger << log4cpp::Priority::DEBUG
                         << "Created AS-to-AS matrix entry!";
    }
    as_as_matrix_mutex.unlock();
...
/* NOTE(Roberto): periodically ouputs and resets the traffic matrix stats */

using boost::asio::ip::udp;

void dump_src_asif_matrix_json(as_nfif_matrix_counters* m, time_t export_time) {
    // export src_asif_matrix


    try {

        int expfile = -1; // tmp file descriptor

        // Prepare UDP socket, in case we need to open it...
        boost::asio::io_service io_service;
        udp::resolver resolver(io_service);
        /* TODO(Roberto): make udp server port configurable */
        udp::resolver::query query(udp::v4(), "localhost", "55555");
        udp::endpoint server = *resolver.resolve(query);
        udp::socket sock(io_service);

        logger << log4cpp::Priority::INFO << "===> Dumping src_asif " 
                                          << m->size() << " objects...";

        if(ixmon_udp_export) {
            sock.open(udp::v4());

            std::string begin_msg("==BEGIN=="); // signals beginning of sequence of JSON export messages
            sock.send_to(boost::asio::buffer(begin_msg), server);
        }
        else {
            char expfile_template[ixmon_expfile_template.length()+1];
            strcpy(expfile_template,ixmon_expfile_template.c_str());
            expfile = mkstemp(expfile_template);
            if(expfile == -1) {
                logger << log4cpp::Priority::ERROR << "ERROR opening ixmon export file: " << expfile_template;
                return;
            }
            logger << log4cpp::Priority::INFO << "Created tmp export file: " << expfile_template;
        }
        ...
        for(auto i = m->begin(); i != m->end(); ++i) {
            ...
            std::string json_msg;
            json_msg += "{ ";
                json_msg += std::string("\"export_time\": ") + std::to_string(export_time) + ", ";
                json_msg += std::string("\"export_timeout\": ") + std::to_string(ix_traffic_matrix_dump_timeout) + ", ";
                json_msg += "\"as_nfif_stats\": { ";
                json_msg += asif.str() + ", ";
                json_msg += counters.str();
            json_msg += " } }";
            if(DEBUG) logger << log4cpp::Priority::DEBUG << json_msg;

            if(ixmon_udp_export) {
                sock.send_to(boost::asio::buffer(json_msg), server);
                // throttle messages to avoid overwhelming the receiver
                boost::this_thread::sleep(boost::posix_time::milliseconds(10));
            }
            else {
                int s = write(expfile,json_msg.c_str(),json_msg.length());
                s = write(expfile,"\n",1);
            }
        }

        if(ixmon_udp_export) {
            std::string end_msg("==END=="); // signals end of sequence of JSON export messages
            sock.send_to(boost::asio::buffer(end_msg), server);
            sock.close();
        }
        else {
            close(expfile);
        ...
        if(DEBUG) logger << log4cpp::Priority::DEBUG << "===> Done dumping src_asif " << m->size() << " objects!";
    }
    catch (std::exception& e) {
        logger << log4cpp::Priority::INFO << "Exception in dump_src_asif_matrix_json: " << e.what();
    }
}

// export as_as_matrix
void dump_as_as_matrix_json(as_as_matrix_counters* m, time_t export_time) {
    ...
    try {
        ...
        if(DEBUG) logger << log4cpp::Priority::DEBUG
                         << "===> Dumping as_as " << m->size() << " objects...";

        if(ixmon_udp_export) {
            sock.open(udp::v4());

            // signals beginning of sequence of JSON export messages
            std::string begin_msg("==BEGIN==");
            sock.send_to(boost::asio::buffer(begin_msg), server);
        }
        else {
            char expfile_template[ixmon_expfile_template.length()+1];
            ...
            if(expfile == -1) {
                logger << log4cpp::Priority::ERROR
                       << "ERROR opening ixmon export file: "
                       << expfile_template;
                return;
            }
            logger << log4cpp::Priority::INFO
                   << "Created tmp export file: "
                   << expfile_template;
        }
        ...
        for(auto i = m->begin(); i != m->end(); ++i) {
            ...
            std::string json_msg;
            json_msg += "{ ";
                json_msg += std::string("\"export_time\": ") + \
                            std::to_string(export_time) + ", ";
                json_msg += std::string("\"export_timeout\": ") + \
                            std::to_string(ix_traffic_matrix_dump_timeout) + \
                            ", ";
                json_msg += "\"srcAS_dstAS_stats\": { ";
                json_msg += asas.str() + ", ";
                json_msg += counters.str();
            json_msg += " } }";
            if(DEBUG) logger << log4cpp::Priority::DEBUG << json_msg;
        ...
        logger << log4cpp::Priority::INFO << "===> Done dumping as_as " << m->size() << " objects!";
    }
    catch (std::exception& e) {
        std::cerr << "Exception in dump_as_as_matrix_json: " << e.what();
    }
}



void ix_traffic_matrix_dump_and_clear() {

    time_t export_time = time(NULL);

    // swap src_asif_matrix_ptr to a "clean" src_asif_matrix
    src_asif_matrix_mutex.lock();
    // points to current matrix, which needs to be cleared
    as_nfif_matrix_counters *src_asif_matrix_to_be_cleared_ptr = 
                                                    src_asif_matrix_ptr;
    src_asif_matrix_active_idx = not src_asif_matrix_active_idx;
    src_asif_matrix_ptr = &src_asif_matrix[src_asif_matrix_active_idx];
    src_asif_matrix_mutex.unlock();

    // swap as_as_matrix_ptr to a "clean" as_as_matrix
    as_as_matrix_mutex.lock();
    // points to current matrix, which needs to be cleared
    as_as_matrix_counters *as_as_matrix_to_be_cleared_ptr = as_as_matrix_ptr;
    as_as_matrix_active_idx = not as_as_matrix_active_idx;
    as_as_matrix_ptr = &as_as_matrix[as_as_matrix_active_idx];
    as_as_matrix_mutex.unlock();

    // no locking needed, because this thread should be the only one that is able
    // to swap the pointers and clear the matrix
    // thus, until clearing is done, the should be no other swap!
    dump_src_asif_matrix_json(src_asif_matrix_to_be_cleared_ptr, export_time);
    src_asif_matrix_to_be_cleared_ptr->clear(); // zero out the data structure

    // dump srcAS-to-dstAS matrix
    dump_as_as_matrix_json(as_as_matrix_to_be_cleared_ptr, export_time);
    as_as_matrix_to_be_cleared_ptr->clear(); // zero out the data structure

}

void dump_ix_traffic_matrix_thread_run() {
    time_t next_timeout = ix_traffic_matrix_dump_timeout;
    while (true) {
        /* TODO(Roberto): change the mechanism below:
            check frequently if time window is over, to avoid possible
            time window inflation due to the time spent to dump and clear
            the matrix
        */

        boost::this_thread::sleep(boost::posix_time::seconds(next_timeout));
        time_t prev_dump_time = time(NULL);
        ix_traffic_matrix_dump_and_clear();

        // seconds to sleep before next dump
        // takes into account how much time was spent inside ix_traffic_matrix_dump_and_clear();
        next_timeout = ix_traffic_matrix_dump_timeout - (time(NULL) - prev_dump_time);
        if(DEBUG) logger << log4cpp::Priority::DEBUG << "========== next traffix matrix dump timeout = " << next_timeout;
    }
}

void update_patricia_ip_asn_tree() {

    patricia_tree_t* new_ip_asn_tree_ptr = build_patricia_ans_tree_from_bgp_rib();

    // swap updated tree
    patricia_tree_t* old_ip_asn_tree_ptr = ip_asn_tree_ptr;
    patricia_ip_asn_tree_mutex.lock();
    ip_asn_tree_ptr = new_ip_asn_tree_ptr;
    patricia_ip_asn_tree_mutex.unlock();

    Destroy_Patricia(old_ip_asn_tree_ptr, (void_fn_t)0);
}

void update_patricia_ip_asn_tree_thread_run() {
    time_t next_timeout = update_patricia_ip_asn_tree_timeout;
    while (true) {
        boost::this_thread::sleep(boost::posix_time::seconds(next_timeout));
        time_t prev_dump_time = time(NULL);
        update_patricia_ip_asn_tree();

        // seconds to sleep before next dump
        // takes into account how much time was spent to concretely update the tree
        next_timeout = update_patricia_ip_asn_tree_timeout - (time(NULL) - prev_dump_time);
        if(DEBUG) logger << log4cpp::Priority::DEBUG << "========== next patricia ip_asn tree update timeout = " << next_timeout;
    }
}
    ...
    for (map_of_vector_counters::iterator itr = SubnetVectorMap.begin(); itr != SubnetVectorMap.end(); ++itr) {
        for (vector_of_counters::iterator vector_itr = itr->second.begin();
            ...
            if (we_should_ban_this_ip(current_average_speed_element, current_ban_settings)) {
                if(DEBUG) logger << log4cpp::Priority::DEBUG << "We have found host group for this host as: " << host_group_name;
...
void traffic_draw_program() {
    ...
    output_buffer << "IXmon " << ixmon_version
...
void init_logging() {
    ...
    logger.setPriority(IXMON_LOG_PRIORITY);
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        if (vm.count("help")) {
            logger << log4cpp::Priority::DEBUG << desc;
            exit(EXIT_SUCCESS);
        }

        if (vm.count("version")) {
            logger << log4cpp::Priority::DEBUG << "Version: " << ixmon_version;
            exit(EXIT_SUCCESS);
        }
        ...
        if (vm.count("configuration_file")) {
            ...
            logger << log4cpp::Priority::DEBUG << "We will use custom path to configuration file: " << global_config_path;
        }

        if (vm.count("log_file")) {
            log_file_path = vm["log_file"].as<std::string>();
            logger << log4cpp::Priority::DEBUG << "We will use custom path to log file: " << log_file_path;
        }
    } catch (po::error& e) {
    ...
    /* NOTE(Roberto): build ip-to-asn patricia tree */
    patricia_ip_asn_tree_mutex.lock(); // redundant, but added just for sure
    ip_asn_tree_ptr = build_patricia_ans_tree_from_bgp_rib();
    patricia_ip_asn_tree_mutex.unlock();
    /**********/
    ...
    //Maria-edits: load the list of networks:
    load_subnet_tracking_ASes();

    //Maria-edits: we don't need that any more.
    ...
    /* NOTE(Roberto): thread for periodically dumping AS-to-PortID matrix */
    ix_traffic_thread_group.add_thread(new boost::thread(dump_ix_traffic_matrix_thread_run));
    ix_traffic_thread_group.add_thread(new boost::thread(update_patricia_ip_asn_tree_thread_run));
    /***********/
    ...
    /* NOTE(Roberto): Wait for asif threads to terminate */
    ix_traffic_thread_group.join_all();
    /**********/
...
void free_up_all_resources() {
    ...
    Destroy_Patricia(ip_asn_tree_ptr, (void_fn_t)0);

    //Maria-edits: clear the hash table.
    subnet_tracking_ASes.clear();
