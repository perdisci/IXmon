...
/* NOTE(Roberto): 
    added third argument to host AS number
*/
patricia_node_t *
patricia_insert_uint32(patricia_tree_t *patricia, prefix_t *prefix, uint32_t uint32_data) {
    ...
    new_node->data = NULL;
    new_node->uint32_data = uint32_data;
    patricia->num_active_node++;
	...
	fprintf (stderr, "patricia_lookup: new_node #3 %s/%d (parent)\n", 
        ...
        glue->data = NULL;
        glue->uint32_data = 0; // FIXME(Roberto): not 100% sure what the uint32_data value in glue should be, probably 0
        patricia->num_active_node++;
...
/* NOTE(Roberto): Wrapper to new paricia_lookup with third argument */
patricia_node_t *
patricia_lookup(patricia_tree_t *patricia, prefix_t *prefix) {
    return patricia_insert_uint32(patricia, prefix, 0);
}
...
/* NOTE(Roberto): adding modified Maria's code */
patricia_node_t *make_and_insert_uint32(patricia_tree_t *tree, const char *string, uint32_t uint32_data) {
    prefix_t *prefix;
    patricia_node_t *node;

    prefix = ascii2prefix (AF_INET, string);
    // printf ("make_and_lookup: %s/%d\n", prefix_toa (prefix), prefix->bitlen);
    node = patricia_insert_uint32(tree, prefix, uint32_data);
    Deref_Prefix (prefix);
    return (node);
}

patricia_node_t *make_and_insert_ipv6_uint32(patricia_tree_t *tree, const char *string, uint32_t uint32_data) {
    prefix_t *prefix;
    patricia_node_t *node;
...
patricia_node_t *
make_and_lookup_ipv6 (patricia_tree_t *tree, const char *string)
{
    prefix_t *prefix;
    patricia_node_t *node;
