...
typedef struct _patricia_node_t {
   ...
   /* NOTE(Roberto): added pointer to integer data 
        to host AS number without using dynamic memory allocation */
...
/* NOTE(Roberto): merging of Maria's code for fast IP-to-AS lookup 
 * Changed function names from "lookup" to "insert" as lookup is really not what happens! */
patricia_node_t *patricia_insert_uint32(patricia_tree_t *patricia, prefix_t *prefix, uint32_t uint32_data);
